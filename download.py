...
async def fetch_data(url: str, headers: dict = None):
    ...
    if headers:
        if 'headers' in headers:
            actual_headers = headers.get('headers')
            proxies = headers.get('proxies')
        else:
    ...
    async with httpx.AsyncClient(proxies=proxies) as client:
        response = await client.get(url, headers=actual_headers, follow_redirects=True)
...
async def fetch_data_stream(url: str, request: Request, headers: dict = None, file_path: str = None):
    ...
    async with httpx.AsyncClient(proxies=proxies) as client:
        # 启用流式请求，并在客户端端点发起请求时跟随 302 重定向
        async with client.stream("GET", url, headers=actual_headers, follow_redirects=True) as response:
    ...
    try:
        ...
        if data_type == 'video':
            ...
            if platform == 'bilibili':
                ...
                video_url = (video_data.get('nwm_video_url_HQ') or video_data.get('nwm_video_url')) if not with_watermark else (video_data.get('wm_video_url_HQ') or video_data.get('wm_video_url'))
                ...
                # 其他平台的常规处理 (增加 nwm_video_url_HQ -> nwm_video_url -> wm_video_url 的回退逻辑)
                video_data = data.get('video_data', {})
                ...
                if not download_url:
                    raise HTTPException(
                        status_code=404,
                        detail="Could not find a valid video play URL for download."
                    )

                success = await fetch_data_stream(download_url, request, headers=__headers, file_path=file_path)
                if not success:
                    raise HTTPException(
                        status_code=500,
        ...
        # 下载图片文件/Download image file
            ...
            for url in urls:
                ...
                response = await fetch_data(url, __headers)
